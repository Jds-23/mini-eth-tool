...
const Calculator = () => {
    ...
    if (lastChanged.current === "binary") {
      if (binary === "") {
        ...
        if (!isNaN(dec)) {
          ...
          setValue("hex", dec.toString(16).toUpperCase(), { shouldTouch: false })
        }
      }
    }
    if (lastChanged.current === "decimal") {
      if (decimal === "") {
        setValue("binary", "")
        setValue("hex", "")
      } else {
  ...
  return (
    <div className="bg-gray-50 p-6 rounded-lg border-2 border-dashed max-w-sm border-black shadow flex flex-col gap-6">
      <h2 className="text-lg font-bold mb-2">Binary / Decimal / Hex Calculator</h2>
      <Form {...form}>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      onChange={e => {
                        const raw = unformatHex(e.target.value ?? "").toUpperCase()
                        field.onChange(raw)
                        lastChanged.current = "hex"
                      }}
                    />
