import { AbiFunction, AbiError, AbiParameters } from 'ox'
import { z } from 'zod'
...
import { useSignatureLookup, type SignatureItem } from '../lib/hooks/useSignatureLookup'
...
export default function Decoder() {
  ...
  const effectiveSig = sig || (usedSig ? usedSig : (sigData && sigData.result.function && Object.values(sigData.result.function).flat()[0]?.name)) || ''
  ...
  function handleDecode() {
    ...
    let hexVal = (hex || '').trim()
    if (hexVal.length < 10) {
      setDecodeError('Hex must be at least 10 characters (0x + 8 hex chars) for function selector')
      return
    }
  ...
  return (
    <div className="bg-gray-50 p-6 rounded-lg border-2 border-dashed max-w-lg border-black shadow flex flex-col gap-6">
      ...
      <Form {...sigForm}>
        <form className="flex flex-col gap-4" onSubmit={e => { e.preventDefault(); handleDecode(); }}>
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <Input {...field} placeholder="Hex data to decode (0x...)" autoComplete="off" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
      ...
      {!sig && selector.length === 10 && (
        <>
          {isLookupLoading && (
            <div className="text-sm text-muted-foreground">Looking up possible signatures...</div>
          )}
          {sigData && sigData.result.function && (
            <div className="flex flex-col gap-2">
              <div className="text-xs text-muted-foreground">Possible signatures from selector:</div>
              {Object.values(sigData.result.function).flat().filter((item): item is SignatureItem => item !== null).map((item, idx) => (
                <button
                  key={item.name + idx}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <Textarea {...field} placeholder="Signature: approve(address,uint) or (uint,address)" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          />
          <Button type="submit" className="w-fit">Decode</Button>
        </form>
      </Form>
        ...
        <div className="flex flex-col gap-2">
          {paramFields.map((field, idx) => (
            <div key={field.name} className="flex gap-2 items-center">
              <span className="text-xs text-muted-foreground w-32">{field.name} ({field.type}):</span>
              <Input type="text" readOnly value={decoded[idx]?.toString?.() ?? ''} className="flex-1" />
            </div>
          ))}
        </div>
      )}
        ...
        <div className="flex flex-col gap-2">
          {decoded.map((val, idx) => (
            <div key={idx} className="flex gap-2 items-center">
              <span className="text-xs text-muted-foreground w-32">arg{idx}:</span>
              <Input type="text" readOnly value={val?.toString?.() ?? ''} className="flex-1" />
            </div>
          ))}
        </div>
      )}
